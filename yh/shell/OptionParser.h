#ifndef YH_SHELL_OPTIONPARSER_H_
#define YH_SHELL_OPTIONPARSER_H_
...
class MultiStringRange
{
    ...
    std::vector< StringArg >::const_iterator cur;
    std::vector< StringArg >::const_iterator end;

  public:
    explicit MultiStringRange(std::vector< StringArg >::const_iterator cur, const std::vector< StringArg >::const_iterator end)
      : cur(cur), end(end) {
//        JS_ASSERT(end - cur >= 0);
    }
    ...
    size_t argno() const { /* S_ASSERT(!empty()); */ return cur->argno; }
};
...
class OptionParser
{
  ...
  private:
    typedef std::vector<detail::Option *> Options;
...
#endif /* YH_SHELL_OPTIONPARSER_H_ */
