...
OptionParser::printHelp(const char *progname)
{
    ...
    if (!arguments.empty()) {
        ...
        size_t lhsLen = 0;
        for (Options::iterator it = arguments.begin(); it != arguments.end(); ++it)
...
OptionParser::handleOption(Option *opt, size_t argc, char **argv, size_t *i, bool *optionsAllowed)
{
    ...
    switch (opt->kind) {
      ...
      case OptionKindMultiString:
      {
        ...
        opt->asMultiStringOption()->strings.push_back(arg);
        return Okay;
      }
      default:
...
{
    if (nextArgument >= arguments.size())
    ...
    switch (arg->kind) {
      ...
      case OptionKindMultiString:
      {
        ...
        arg->asMultiStringOption()->strings.push_back(value);
        return Okay;
      }
      default:
...
{
    int i=0;
    for (Options::const_iterator it = arguments.begin(); it != arguments.end(); ++it,++i) {
        const char *target = (*it)->longflag;
...
{
//    if (!options.reserve(options.size() + 1))
//        return false;
    IntOption *io = new IntOption(shortflag, longflag, help, metavar, defaultValue);
    if (!io)
        return false;
    options.push_back(io);
    return true;
}
...
//        return false;
    BoolOption *bo = new BoolOption(shortflag, longflag, help);
    if (!bo)
        return false;
    options.push_back(bo);
    return true;
}
...
//    if (!options.reserve(options.length() + 1))
//        return false;
    StringOption *so = new StringOption(shortflag, longflag, help, metavar);
    if (!so)
        return false;
...
//    if (!options.reserve(options.length() + 1))
//        return false;
    MultiStringOption *mso = new MultiStringOption(shortflag, longflag, help, metavar);
    if (!mso)
        return false;
