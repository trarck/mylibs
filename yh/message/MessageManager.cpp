...
#if YH_ENABLE_SCRIPT_BINDING
	#define MESSAGE_GETOBJECT_ID(obj) (obj)->getId()
#else
...
,_nullObject(NULL)
{
	//YHDEBUG("MessageManager create");
}

MessageManager::~MessageManager()
{
	//YHDEBUG("MessageManager destroy begin");
	YH_SAFE_RELEASE_NULL(_globalObject);
		
	YH_SAFE_RELEASE_NULL(_nullObject);
	//YH_SAFE_RELEASE(_regiesterMap);
	//YHDEBUG("MessageManager destroy end");
}
...
{
	YH_SAFE_RELEASE_NULL(s_sharedMessageManagerInstance);
}
...
{
	YHASSERT(receiver!=NULL,"MessageManage:registerReceiver:receiver can't be null");
	YHASSERT(handle!=NULL,"MessageManage:registerReceiver:handle");
	YHASSERT(handleObject!=NULL,"MessageManage:registerReceiver:handleObject");
	...
	for(HandleList::iterator iter=_messages[type][receiver][sender].begin();iter!=_messages[type][receiver][sender].end();++iter){
		...
		if (handler->getHandle()==handle && handler->getTarget()==handleObject) {
			YHASSERT(0,"Handle has register");
            isRegisted=true;
            break;
		}
	}
...
bool MessageManager::registerReceiver(Ref* receiver ,unsigned int type ,Ref* sender,MessageHandler* handler)
{
	...
	YHASSERT(receiver!=NULL,"MessageManage:registerReceiver:receiver can't be null");
	YHASSERT(handler!=NULL,"MessageManage:registerReceiver:handler");
...
{
    YHASSERT(receiver!=NULL,"MessageManager:removeReceiver:receiver can't be null!");
	YHASSERT(handle!=NULL,"MessageManager:removeReceiver:handle can't be null!");
    YHASSERT(handleObject!=NULL,"MessageManager:removeReceiver:handleObject can't be null!");
...
{
    YHASSERT(receiver!=NULL,"MessageManager:removeReceiver:receiver can't be null!");
	YHASSERT(sender!=NULL,"MessageManager:removeReceiver:sender can't be null!");
	YHASSERT(handle!=NULL,"MessageManager:removeReceiver:handle can't be null!");
...
{
	YHASSERT(message!=NULL,"MessageManager:dispatchMessage:message can't be null!");

	//YHASSERT(message.type!=0,)
...
{
	YHASSERT(message!=NULL,"MessageManager:dispatchMessage:message can't be null!");
	YHASSERT(receiver!=NULL,"MessageManager:dispatchMessage:receiver can't be null!");
...
{
	//YHDEBUG("message size:%d", _messages.size());
	//int n = 0;
